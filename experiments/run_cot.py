...
from prompts.templates import cot_prompt
...
def run(model_name: str, dataset_name: str, data_path: str, output_path: str):
    ...
    print(f"\nRunning Zero-shot CoT Prompting | {model_name} | {dataset_name}")
    print("-" * 60)

    for item in tqdm(data, desc="Evaluating"):
        choices = item.get("choices", None)
        prompt = cot_prompt(item["question"], choices)
...
if __name__ == "__main__":
    ...
    for dataset, data_path in [
        ...
        output_path = f"results/{dataset}_{MODEL_NAME}_cot.csv"
